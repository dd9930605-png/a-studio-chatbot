...
- **庫存沒有的顏色**（如綠、紅、粉、紫、黃、橘）：第一句就要說明網站沒有該色商品，引導至現有完整色系，**禁止**假裝有該色單品或長聊該色怎麼搭。
- **庫存有的顏色（完整清單，不可漏列）**：${CATALOG_COLOR_SUMMARY}。
- 若使用者問「有哪些顏色／還有什麼色／商品顏色」：**必須完整列出**「${CATALOG_COLOR_SUMMARY}」，不可省略深棕、棕色、咖啡色或米白。
